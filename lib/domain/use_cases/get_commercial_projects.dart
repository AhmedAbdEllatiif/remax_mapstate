import 'package:dartz/dartz.dart';
import 'package:remax_mapstate/domain/entities/app_error.dart';
import 'package:remax_mapstate/domain/entities/params/area_projects_param.dart';
import '../entities/params/no_params.dart';
import 'package:remax_mapstate/domain/entities/project_entity.dart';
import 'package:remax_mapstate/domain/repositories/api_repository.dart';
import 'package:remax_mapstate/domain/use_cases/use_case.dart';

class GetCommercialProjectsCase extends UseCase<List<ProjectEntity>, AreaProjectsParam> {
  final RemoteRepository apiRepo;

  GetCommercialProjectsCase({required this.apiRepo});

  @override
  Future<Either<AppError, List<ProjectEntity>>> call(AreaProjectsParam params) async =>
      await apiRepo.getCommercialProjects(params.areaId);
}
