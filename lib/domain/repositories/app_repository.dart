...
class AppRepository{
  ...
  /// To open the map
  Future<Either<AppError,void>> openMap(
      {required LocationEntity locationEntity}) async {

    final availableMaps = await MapLauncher.installedMaps;
    print('AvailableMaps:  $availableMaps');

    // launch(urlString)
    // final latitude = locationEntity.;
    ...
    try{
      return Right(await availableMaps.first.showMarker(
        coords: Coords(locationEntity.latitude, locationEntity.longitude),
        title: "Ocean Beach",
      ));
    }on Exception catch(e){
      return Left(AppError(AppErrorType.canOpenWhatsapp,message: "Error: $e"));
